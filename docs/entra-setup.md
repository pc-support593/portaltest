...
3. 「APIのアクセス許可」で以下の **委任されたアクセス許可** を追加:
   ...
   | `User.Read.All` | 社内メンバー検索(予約フォーム)+組織図(department属性による部門ごとのグループ表示)。2026-08-22に`User.ReadBasic.All`から引き上げ |
...
3. 権限別にできること:
   ...
   | `Calendars.ReadWrite.Shared` 等の管理者同意(§1) | グローバル管理者 / 特権ロール管理者 / クラウド アプリケーション管理者 |
