...
- `public/organization.html` / `public/js/organization.js` — 組織図画面。**2026-09-08にツリー構造(階層図)へ変更(ユーザー指示)**: department(部署名の文字列一致)ベースの手動対象部門一覧はやめ、**Entra IDの`manager`属性(上長)を正として全社の階層を自動構築**する。`fetchOrgUsers()`がGraph `/users`一覧(`$filter`+`@odata.nextLink`でページング)を取得し、各ユーザーのマネージャーは`/users/{id}/manager`で個別取得する(同時実行数8件に絞って取得。**2026-09-08修正**: 当初`$expand=manager`を一覧取得に付けていたが、advanced query(`$filter`のendsWith+`$count`)との組み合わせでHTTP 400になったため撤回し、この方式に変更した。manager未設定のユーザーは`/manager`が404を返す仕様のため、404はエラーではなく「マネージャーなし」として扱う)。対象は検索機能(§common.js)と同じ`@yoshimuraichi.com`/`@yumesumika.com`の2ドメインに加えて`accountEnabled eq true`で絞り込み(会議室・社用車等のリソースメールボックスはサインイン無効=`accountEnabled:false`のため除外。人だけを対象にするため)。追加のGraph権限は不要(`User.Read.All`の範囲内。manager/directReportsの読み取りに新規スコープは不要)。`buildOrgTree(users)`が`managerId`から親子関係を構築(マネージャー未設定、またはマネージャーが対象2ドメイン外の場合はルート=トップ階層扱い)。表示は`renderNode`によるインデント付きの折りたたみツリー(▸/▾クリックで開閉、`state.expanded`で開閉状態を保持)。初期表示はルートとその直下(部門長クラス)のみ展開し、それより下は折りたたむ(全社員一度表示だと縦に長大になるため)。**既知の制約**: Entra ID側で「マネージャー」が未設定の社員が多いと階層にならずトップ階層に多数並ぶ。この状態(ルート数が全体の3割以上)を検知すると、Entra ID側の設定を促す案内バナーを表示する。devモードは非対応(案内文のみ)。自動リフレッシュはルール11に準拠(開閉状態は保持したまま中身だけ差し替え)。参考資料として実際の組織図(PDF/Excel)を`_governance/reference/組織図.xlsx`等で受領済み(Gitリポジトリ外・個人情報のため`Portal/`配下には置かない。Excel自体は表形式ではなく手作業配置の図のためプログラムでの解析はしていない)
...
9. Graph権限は都度最小限を追加する(現在: `User.Read`, `Calendars.ReadWrite`, `User.Read.All`(2026-08-22に`User.ReadBasic.All`から引き上げ。社内メンバー検索の部署表示+組織図画面のツリー構築=`fetchOrgUsers`用。manager/directReportsの読み取りに新規スコープは不要)、`Calendars.ReadWrite.Shared`(2026-08-22時点で用途が拡大: 拠点代表者の会議室削除機能=§12、2026-08-24以降の実予約表示・作成=rooms.js、**予約状況の全表示(件名・予約者名の取得)=schedule.jsの`fetchCalendarViewBusy`で全会議室・全社用車が対象**)、自アプリの `access_as_user`。`Place.Read.All`は不使用)。このテナントは**低リスク権限でも管理者の同意が必須**な設定になっているため、権限追加のたびに管理者に同意実行を依頼する
