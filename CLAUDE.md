...
12. **拠点代表者による会議室予約の削除機能**: `schedule.js` 冒頭の `SITE_REPS`(拠点ID→担当者メール配列)を直接編集して運用する。担当拠点は `siteGridHtml` に「担当拠点」バッジ+各予約に削除ボタンが出る。**2026-08-22時点で件名・予約者名の表示自体はSITE_REPS不要で全員に見えるようになった**ため、この機能が担うのは「削除ボタンを誰に出すか」のみ。**① `SITE_REPS` は入力済み(2026-08-22・ユーザー指示)**: 5拠点共通で`COMMON_SITE_REPS`(k-iwatani@yumesumika.com, m-sakahara@yumesumika.com, y-nishida@yumesumika.com)を設定。**未完了の前提条件はあと1つ**: ② Exchange側で上記3名に各会議室カレンダーの**編集**権限を付与(`Add-MailboxFolderPermission -Identity "<会議室>:\Calendar" -User <担当者> -AccessRights Editor`。Reviewer権限だけでは削除できない)。③のEntra同意は§14で完了済み
