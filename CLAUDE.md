...
- `public/organization.html` / `public/js/organization.js` — **社員名簿画面**(旧称「組織図」。2026-09-10にユーザー指示で名称変更。URLは引き続き`organization.html`)。**department属性による部門ごとのグループ表示**(氏名・メール・電話番号)。
  ...
  - **2列表示(2026-09-10・ユーザー指示)**: メールアドレスが`@yumesumika.com`の社員は右列(`#org-right`。見出し「ゆめすみか」)、それ以外(`@yoshimuraichi.com`)は左列(`#org-left`。見出し「吉村一建設」)に振り分ける(`splitByCompany`)
  - `buildGroups(users, priorityTitles)`が各列ごとに`department`属性でグループ化。**優先グループ(部門扱いにして先頭表示)は2種類**: ①役職(`jobTitle`)に「会長」「社長」「専務」を含む社員(`PRIORITY_TITLES`。左列のみ適用。会長→社長→専務の順、該当者がいるものだけ)、②`NAME_OVERRIDES`(氏名一致による個別対応。姓名間のスペース有無は`normalizeName`で吸収。2026-09-10時点は「森下直美」→「ゆめすみか常務取締役」のみ。右列側で①と同様の先頭見出し扱いになる。表示名を変更した場合はこのMapのキーも直すこと)。`department`が空欄の社員は(上記いずれにも該当しない限り)表示しない(旧「(部門未設定)」バケットは廃止)
  - `DEPARTMENT_OVERRIDES`(メールアドレスの@より前の部分の完全一致・大文字小文字区別なしによる個別対応。2026-09-10時点は`naofumi_kotani`→「設計企画部」のみ。Entra ID側の`department`属性が実態と異なる/未設定の社員を、実際の所属部門に強制的に振り分けるために使う。優先グループには該当しない通常の部門グループとして扱われる=先頭表示にはならない)
