...
社員向け社内ポータル。5画面: ポータルトップ(お知らせ・クイックリンク・社内規程・予定)/ 会議室予約(拠点別カレンダー。**2026-07-01〜08-23はデザインサンプル、2026-08-24以降はentraモードで実際のExchange連携**)/ スケジュール(個人 + 拠点別会議室、**両方実データ**)/ 社員名簿(旧称「組織図」。部門別メンバー一覧。**実データ=Entra ID**。2026-08-22追加、2026-09-10に名称変更)/ 管理画面(コンテンツCRUD)。
...
- `public/js/common.js` — 全画面共通ユーティリティ(`api()` / `esc()` 等)。`searchMembers(q)`(社内メンバー検索。entraモードはGraph `/users`実データ、devモードはダミー名簿`/api/users`)を`rooms.js`と`schedule.js`で共用。**entraモードは`@yoshimuraichi.com`/`@yumesumika.com`の2ドメインに`$filter`で絞り込み済み**(ユーザー指示 2026-09-07。他ドメイン・外部ゲストアカウント等は検索結果から除外)。`PORTAL_PAGES`/`searchPortalPages(q)`(ポータル内ページのタイトル・キーワード一覧と部分一致検索。2026-09-07追加。新しい画面を追加したら`PORTAL_PAGES`にも追記する)
...
- `public/organization.html` / `public/js/organization.js` — **社員名簿画面**(旧称「組織図」。2026-09-10にユーザー指示で名称変更。URLは引き続き`organization.html`)。**department属性による部門ごとのグループ表示**(氏名・メール・電話番号)。
  ...
  - **2列表示(2026-09-10・ユーザー指示)**: メールアドレスが`@yumesumika.com`の社員は右列(`#org-right`。見出し「ゆめすみか」)、それ以外(`@yoshimuraichi.com`)は左列(`#org-left`。見出し「吉村一建設」)に振り分ける(`splitByCompany`)。**メールアドレス一致(大文字小文字区別なし)を氏名一致より優先して使う理由**: 表示名は姓名間のスペース有無等で表記ゆれが起きやすく、実際に「森下直美」への対応が一致せず反映されなかった不具合(2026-09-10)があったため、個人特定はすべてメールアドレスで行う方針にした
  - `buildGroups(users, priorityTitles)`が各列ごとに`department`属性でグループ化。**優先グループ(部門扱いにして先頭表示)は2種類**: ①役職(`jobTitle`)に「会長」「社長」「専務」を含む社員(`PRIORITY_TITLES`。左列のみ適用。会長→社長→専務の順、該当者がいるものだけ)、②`PERSON_OVERRIDES`(メールアドレス指定による個別対応。`{column, group}`のうち`group`を指定した人だけ優先グループ扱いになる。2026-09-10時点: `n-morishita@yoshimuraichi.com`→`{column:'right', group:'ゆめすみか常務取締役'}`=メールドメインは`@yoshimuraichi.com`のままだが右列の先頭見出しに、`s-tada@yumesumika.com`/`katsu-oshima@yumesumika.com`/`s-yamanaka@yumesumika.com`→`{column:'left'}`(groupなし)=メールドメインは`@yumesumika.com`だが左列に振り分け、部門名は実際の`department`属性(不動産部・㈱来夢エンジニア)のまま通常の部門グループとして扱う。**`splitByCompany`が列指定をメールドメインより優先する**ため、これらの対象者は右列側の該当部門グループから自動的にいなくなる=見た目上「削除」される)。`department`が空欄の社員は(上記いずれにも該当しない限り)表示しない(旧「(部門未設定)」バケットは廃止)
  - `DEPARTMENT_OVERRIDES`(メールアドレスの完全一致・大文字小文字区別なしによる個別対応。2026-09-10時点: `naofumi_kotani@yumesumika.com`/`kotani@yoshimuraichi.com`→どちらも「設計企画部」。Entra ID側の`department`属性が実態と異なる/未設定の社員を、実際の所属部門に強制的に振り分けるために使う。優先グループには該当しない通常の部門グループとして扱われる=先頭表示にはならない。列(左右)はメールドメインに従うため、この2人は別々の列の「設計企画部」に分かれて表示される)
  - **個人特定を追加するときの注意**: `PERSON_OVERRIDES`/`DEPARTMENT_OVERRIDES`のキーは必ず本人のEntra ID上の実際のメールアドレス(小文字)を使うこと。氏名では表記ゆれで一致しない事故が起きるため、氏名をキーにした特別対応は今後追加しないこと
  - **部門内の並び順**: `sortKeyFromEmail(email)`(メールアドレスの最初の「-」より後ろの部分=ローマ字の姓。2026-09-10・ユーザー指示)を取り出し、`compareRomajiGojuon(a, b)`(2026-09-17追加)で比較する。**漢字の氏名を`localeCompare('ja')`で並べても正しい五十音順にはならないことを実機検証済み**(「友藤/東/森本/千葉/巽/森下/小谷」→`小谷,森下,森本,千葉,巽,東,友藤`となり、本来の読み順`小谷,巽,千葉,友藤,東,森下,森本`と一致しなかった。Entra IDにふりがな属性が無いため)。このテナントのメールアドレス命名規則(頭文字-姓のローマ字。例: `n-morishita`)を五十音順の代用として利用している。「-」が無いメールアドレス(`naofumi_kotani@…`等)は@より前の全体を代わりに使う。
...
9. Graph権限は都度最小限を追加する(現在: `User.Read`, `Calendars.ReadWrite`, `User.Read.All`(2026-08-22に`User.ReadBasic.All`から引き上げ。社内メンバー検索の部署表示+社員名簿画面(旧組織図)の`fetchOrgUsers`用)、`Calendars.ReadWrite.Shared`(2026-08-22時点で用途が拡大: 拠点代表者の会議室削除機能=§12、2026-08-24以降の実予約表示・作成=rooms.js、**予約状況の全表示(件名・予約者名の取得)=schedule.jsの`fetchCalendarViewBusy`で全会議室・全社用車が対象**)、自アプリの `access_as_user`。`Place.Read.All`は不使用)。このテナントは**低リスク権限でも管理者の同意が必須**な設定になっているため、権限追加のたびに管理者に同意実行を依頼する
10. ~~社内メンバー検索(User.ReadBasic.All)は基本プロフィールのみで部署(department)は取得できない~~ → **2026-08-22廃止(ユーザー指示)**。社員名簿画面(旧組織図)のため`User.Read.All`に引き上げ済み。社内メンバー検索・社員名簿とも部署情報を表示する
11. **自動リフレッシュの共通方針(ユーザー承認・2026-08-20)**: 動的データの表示は「2分間隔で裏側から再取得・モーダル表示中と非表示タブ(document.hidden)はスキップ・差分があるときだけ静かに差し替え(自動更新時はローディング表示を出さない)・失敗は静かに無視して次回再試行」。実装済み: portal.js(今日の予定)・schedule.js(個人+拠点別)・rooms.js(サンプル予約)・organization.js(社員名簿。旧称組織図。2026-08-22追加)。**今後、動的表示を新設するときも同方針を適用する**
