...
- `public/organization.html` / `public/js/organization.js` — **社員名簿画面**(旧称「組織図」。2026-09-10にユーザー指示で名称変更。URLは引き続き`organization.html`)。**department属性による部門ごとのグループ表示**(氏名・メール・電話番号)。
  ...
  - `DEPARTMENT_OVERRIDES`(メールアドレスの完全一致・大文字小文字区別なしによる個別対応。2026-09-10時点は`naofumi_kotani@yumesumika.com`→「設計企画部」のみ。Entra ID側の`department`属性が実態と異なる/未設定の社員を、実際の所属部門に強制的に振り分けるために使う。優先グループには該当しない通常の部門グループとして扱われる=先頭表示にはならない)
