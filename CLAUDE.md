...
- `public/js/rooms.js` — 会議室予約の全ロジック(カレンダー、予約フォーム、CSV出力)。**拠点・会議室名は実際のExchange会議室(roomsData.jsのマスタ)を表示**。**2026-08-24以降の予約は、entraモードでは実際にExchangeと連携する(ユーザー指示・2026-08-21。schedule.jsと同じ`/me/events`+会議室resource出席者の仕組み。`REAL_ROOMS_ENABLED`フラグでON/OFF可)**。devモードは従来どおりサーバーSQLite保存のサンプル動作(`src/db.js`のシードもそのまま機能する)。
